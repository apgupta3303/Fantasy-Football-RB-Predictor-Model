...
```{r}
Ekeler <- select(filter(fantasy_model_test, player_name == "A.Ekeler"), player_name, week, season, fantasy_points, predictedFantasyPoints)
Ekeler
mean(Ekeler$fantasy_points)
mean(Ekeler$predictedFantasyPoints)
plot(Ekeler$week,Ekeler$fantasy_points,type="l",col="red")
par(new = TRUE) %>%
plot(Ekeler$week,Ekeler$predictedFantasyPoints,type="l",col="green")
lines(Ekeler$week,Ekeler$predictedFantasyPoints,col="green")
ggplot(Ekeler, aes(x = week, y = c(fantasy_points, predictedFantasyPoints)) + geom_point())

ggplot(Ekeler, aes(week)) +                   
  geom_point(aes(y= fantasy_points), color="blue") +  
  geom_point(aes(y= predictedFantasyPoints), color="red") +
  ylab("Points") +
  ggtitle("Ekeler Points vs Predicted Points")

```

```{r}
Javonte <- select(filter(fantasy_model_test, team == "Broncos", player_name == "J.Williams", week == 13), predictedFantasyPoints, fantasy_points)
Javonte
p <-ggplot(data=Javonte, aes(x= predictedFantasyPoints, fantasy_points)) +
  geom_bar(stat="identity")
barplot(as.matrix(Javonte), col= "blue",
        ylab = "Points", main = "Javonte Williams Week 13 Prediction")
```


```{r}
mean((filter(fantasy_model_test, player_name == "N.Harris")$predictedFantasyPoints - filter(fantasy_model_test, player_name == "N.Harris")$fantasy_points)^2)

Harris <- filter(fantasy_model_test, player_name == "N.Harris")
ggplot(Harris, aes(week)) +                   
  geom_point(aes(y= fantasy_points), color="blue") +  
  geom_point(aes(y= predictedFantasyPoints), color="red") +
  ylab("Points") +
  ggtitle("Najee Harris Points vs Predicted Points")
```
...
```{r}
ggplot(expected_carries, mapping = aes(x = expected_carries$carries, y = expected_carries$fantasy_points)) + geom_point() +
xlab("carries") + ylab("Fantasy Points") + ggtitle("Carries vs Fantasy Points (2019)")
```
For our model, we are trying to predict a RBs fantasy points in a given week. This graph
shows a correlation between the number of carries a RB gets and the number of 
fantasy points that they score. This makes sense because the more carries they 
get, the more opportunities they have to receive fantasy points. This information
led us to breaking our model down into 3 different models: expected carries for
that week, expected yards per carry for that week, expected TDs for that week.

```{r}
D_Hen <- filter(expected_carries, player_name == "D.Henry")
D_Hen
ggplot(D_Hen, mapping = aes(x = D_Hen$spread, y = D_Hen$carries)) + geom_point() +
xlab("spread") + ylab("carries") + ggtitle("Derick Henry Spread vs Carries (2019)")
```

This graph takes Derick Henry's games from 2019 and plots his carries as a 
function of the pregame spread. While it is a small sample size and the correlation
is not perfect, there is  a general downward sloping trend. This makes sense
because the lower the spread is for your team, the more likely your team is to 
win. If you team is winning, then your team is more likely for you to run the 
ball and for a player to get more carries. Therefore, when trying to predict 
carries, our model should take into account the spread before the game.



```{r}
ggplot(expected_carries, mapping = aes(x = expected_carries$avgCarriesSF, y = expected_carries$carries)) + geom_point() +
xlab("AVG Carries up to Week") + ylab("carries") + ggtitle("AVG Carries Up Top Week vs Carries (2019)")
```
This graph shows the correlation between a players average carries up to that 
week and the amount of carries that they receive that week. This makes sense 
to use for our model because taking into account their average carries up to 
that week can help us predict their number of carries in that game.

```{r}
write.csv(expected_carries, file = "ExpectedCarries.csv")
```
